...
import {
  getPaystackPlanCode,
  isPaystackTestConfigurationReady,
  paystackRequest,
} from "@/lib/paystack";
...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      return jsonError(
        "The checkout request was not valid JSON.",
        400,
        "INVALID_REQUEST",
      );
    }
    ...
    const initialized = await paystackRequest<{
      ...
      body: JSON.stringify({
        ...
        currency: "NGN",
        plan: getPaystackPlanCode(planId),
    ...
    if (!initialized.authorization_url)
      return jsonError(
        "Paystack did not return a checkout address.",
        502,
    ...
    return jsonError(
      "Payment setup is not complete yet. Please try again later.",
      503,
