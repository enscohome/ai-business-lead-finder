...
CREATE TABLE public.freelancer_private_details (
...
CREATE TABLE public.freelancer_username_history (
...
CREATE TABLE public.freelancer_social_links (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(), freelancer_id UUID NOT NULL REFERENCES public.freelancer_profiles(id) ON DELETE CASCADE,
...
CREATE TABLE public.freelancer_review_requests (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(), freelancer_id UUID NOT NULL REFERENCES public.freelancer_profiles(id) ON DELETE CASCADE,
...
CREATE TABLE public.freelancer_review_reports (
...
CREATE TABLE public.freelancer_verification_applications (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(), freelancer_id UUID NOT NULL REFERENCES public.freelancer_profiles(id) ON DELETE CASCADE,
...
CREATE TABLE public.freelancer_security_events (
...
CREATE INDEX freelancer_security_events_limit_idx ON public.freelancer_security_events(event_key, subject, ip_hash, created_at DESC);
CREATE INDEX freelancer_portfolio_order_idx ON public.freelancer_portfolio_projects(freelancer_id, display_order);
CREATE INDEX freelancer_reviews_public_idx ON public.freelancer_reviews(freelancer_id, moderation_status, created_at DESC);
CREATE INDEX freelancer_review_requests_owner_idx ON public.freelancer_review_requests(freelancer_id, status);
