...
export async function ensureSubscriptionProfile(supabase: any, user: User) {
  ...
  const allowedStatus =
    ...
    (profile.subscription_status === "cancelled" && paidThrough);
  const expired =
    Boolean(profile.subscription_status && !allowedStatus) ||
    Boolean(profile.subscription_current_period_end && !paidThrough);
  ...
  const periodEnded =
    ...
    (!profile.usage_period_end || new Date(profile.usage_period_end) <= now);
